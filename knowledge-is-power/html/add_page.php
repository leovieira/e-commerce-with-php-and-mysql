...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (!empty($_POST['title'])) {
		...
		$add_page_erros['title'] = 'Please enter the title!';
	}

	if (filter_var($_POST['category'], FILTER_VALIDATE_INT, array('min_range' => 1))) {
		$cat = $_POST['category'];
	} else { // Nenhuma categoria foi selecionada
	...
	if (!empty($_POST['content'])) {
		...
		$add_page_erros['content'] = 'Please enter the content!';
	}
	...
	if (empty($add_page_errors)) {
		...
		if (mysqli_affected_rows($dbc) == 1) {
			...
			$_POST = array();
		} else { // Se não ocorreu corretamente
